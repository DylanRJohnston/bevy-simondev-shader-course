...
fn vertex(vertex: Vertex) -> VertexOutput {
  ...
  local_position += vertex.normal * remap(sin(globals.time * 1.5 + local_position.y * 10), -1.0, 1.0, -0.5, 0.5);

  let world_from_local = mesh_functions::get_world_from_local(vertex.instance_index);

  out.world_normal = mesh_functions::mesh_normal_local_to_world(vertex.normal, vertex.instance_index);
  out.world_position = mesh_functions::mesh_position_local_to_world(world_from_local, vec4f(local_position, 1.0));
...
fn fragment(mesh: VertexOutput) -> @location(0) vec4f { 
  ...
  let normal = normalize(cross(dpdyFine(mesh.world_position.xyz), dpdxFine(mesh.world_position.xyz)));
